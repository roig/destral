...
void update_ball(entt::registry& r) {
	...
	for (auto e : balls_view) {
		...
		//AP_TRACE("%f %f", delta.x, delta.y);
		if (hit.blocking) {
			// Reflection vector
			b.dir = b.dir - 2.0f * glm::dot(b.dir, hit.normal) * hit.normal;
			AP_TRACE("%f %f     %f %f", b.dir.x, b.dir.y, hit.normal.x, hit.normal.y);
...
entt::entity create_block(entt::registry& r, glm::vec2 position) {
	...
	auto block_ppu = 3.f;
	auto block_e = ds::create_collider_rect(r, block_size / block_ppu / 2.0f, position);
...
void ak_init(entt::registry* r) {
	...
	// CREATE BLOCKS
	{
		create_block(*r, { 0,300 });
		create_block(*r, { 300,300 });
	...
	// CREATE THE BALL
	{
		...
		auto ball_ppu = 2.5f;
		auto ball_e = ds::create_collider_circle(*r, ball_radius/ ball_ppu /2.f, { 0, 0 });
